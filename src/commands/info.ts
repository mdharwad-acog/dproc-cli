    ...
    .action(async (bundlePath: string, options: any) => {
      ...
      try {
        ...
        const fieldRows = Object.entries(bundle.stats.fieldStats).map(([field, stats]) => {
          ...
          return {
            Field: field,
            Type: fieldStats.type || 'unknown',
            Unique: (fieldStats.uniqueCount || 0).toString(),
            Nulls: (fieldStats.nullCount || 0).toString(),
            'Null %': (((fieldStats.nullCount || 0) / bundle.metadata.recordCount) * 100).toFixed(1) + '%',
          };
        });
        ...
        if (options.samples) {
          ...
          Display.table(bundle.samples.main);
        }
