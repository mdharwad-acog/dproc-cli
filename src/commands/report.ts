...
export function createReportCommand(): Command {
  ...
  command
    ...
    .argument(
      "[spec]",
      "Optional: Report specification YAML file (for advanced custom reports)"
    )
    ...
    .action(
      async (
        ...
        try {
          ...
          const outputPath =
            options.output ||
            `./output/${basename(bundlePath, ".bundle.json")}-report.md`;
          ...
          if (specPath) {
            ...
            // Simple: auto report with minimal options
            spinner.start("Generating report...");
            log.info("Using auto-report mode");
            ...
            report = await ReportEngine.generateAuto(bundle, reportOptions);
          }
          ...
          if (options.json) {
            ...
            const details: Record<string, string> = {
              ...
              Output: outputPath,
              Size: `${Math.round(
                Buffer.byteLength(report.content) / 1024
              )} KB`,
              Records: (
                report.metadata.recordCount ?? bundle.metadata.recordCount
              ).toLocaleString(),
            };
