...
import { ConfigManager, BundleBuilder, BundleLoader } from "@aganitha/dproc";
import {
  CsvConnector,
  JsonConnector,
  XmlConnector,
  ...
  command
    ...
    .action(async (filePath: string, options: any) => {
      ...
      try {
        ...
        const outputDir =
          options.output || config.defaultOutputDir || "./output";
        await mkdir(outputDir, { recursive: true });

        // Determine bundle name
        const bundleName =
          options.name || basename(filePath).replace(/\.[^/.]+$/, "");
        const outputPath = `${outputDir}/${bundleName}.bundle.json`;

        // Read file using appropriate connector
        spinner.start("Reading data...");
        const ext = extname(filePath).toLowerCase();

        let connector;
        // Fixed: Type the format explicitly
        let format: "csv" | "json" | "xml" | "parquet" | "unknown" = "unknown";
        ...
        switch (ext) {
          ...
          case ".xml":
            ...
            break;
          case ".parquet":
            connector = new ParquetConnector();
            format = "parquet";
            break;
          default:
        ...
        spinner.succeed(
          `Read ${Array.isArray(records) ? records.length : 1} records`
        );
        ...
        const fieldHeaders = ["Field", "Type", "Unique", "Nulls"];
        const fieldRows = Object.entries(bundle.stats.fieldStats).map(
          ([field, stats]) => [
            field,
            (stats as any).type || "unknown",
            ((stats as any).uniqueCount || 0).toString(),
            ((stats as any).nullCount || 0).toString(),
          ]
        );
